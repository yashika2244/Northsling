...
//         "Agent Portal",
//         "Field Sales Employee and Agent Management Application",
//         "Claims Management Solutions",
//         "Risk Assessment Models",
//         "Policyholder Engagement PlatformsSmart Underwriting",
...
//     "Life Sciences": {
//       industries: [
//         "HCP and Patient Engagement Platforms",
//         "Manufacturing and Supply Chain Operations",
//         "Omnichannel Marketing & HCP Engagement",
//         "Advance Data Management and Analytics",
//         "API Customer Collaboration",
//         "Analytics Driven Market Access & Strategy",
...
//   const [active, setActive] = useState("Banking &  Finance");
...
export const data = {
  ...
  Insurance: {
    industries: [
      ...
      "Claims Management Solutions",
      "Risk Assessment Models",
      "Policyholder Engagement Platforms Smart Underwriting",
      "Digital Policy Management",
      "Fraud Detection and Prevention",
    ],
    image:
...
export default function IndustriesPanel() {
  ...
  const getActiveFromURL = () => {
    ...
    for (let cat of categories) {
      const industries = data[cat].industries.map(
        (i) => `/industries/${normalize(i)}`
      );
      if (industries.includes(path)) return cat;
    }
    return "Banking & Finance"; // fallback
  };
  ...
  return (
    <div className="relative hide-scrollbar bg-gradient-to-br from-white via-orange-50 to-white text-gray-800 rounded-3xl shadow-xl border border-orange-100 flex flex-col sm:flex-row overflow-hidden max-w-8xl md:mx-6 mb-16">
      ...
      {/* Desktop Sidebar */}
      <div className="hidden sm:flex sm:w-1/3 flex-col bg-white/60 backdrop-blur-xl border-b sm:border-b-0 border-orange-100 p-5 sm:p-4 rounded-l-3xl relative overflow-hidden">
        <h2 className="text-2xl font-bold mb-3 bg-gradient-to-r from-orange-600 to-pink-500 bg-clip-text text-transparent tracking-tight">
          Industries
        </h2>
        <div className="flex flex-col space-y-2 overflow-y-auto max-h-[480px] hide-scrollbar py-4 px-2 rounded-lg border pr-2 border-gray-100">
          {categories.map((cat) => (
            <motion.button
              key={cat}
              whileHover={{ scale: 1.02, x: 5 }}
      ...
      <div className="flex-1 relative bg-gradient-to-br from-white/90 via-orange-50/40 to-white/80 backdrop-blur-xl p-6 sm:p-10 grid sm:grid-cols-2 gap-10 items-start rounded-3xl">
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="space-y-3">
            {data[active]?.industries.map((industry, i) => {
            const route = `/industries/${normalize(industry)}`;
              return (
                <motion.div
                  key={industry}
                  initial={{ opacity: 0, x: -15 }}
