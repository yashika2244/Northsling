  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (location.pathname === "/") {
        ...
        setScrolled(true); // always solid color on other pages
      }
    };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between py-2">
        ...
        <Link to="/" className="flex items-center space-x-2">
          <div
            className={`h-14 w-14 flex items-center justify-center rounded-full font-bold 
        ...
        <nav
          className={`hidden md:flex justify-center items-center rounded-full py-2 px-1 shadow-sm cursor-pointer transition-all duration-300 ${
            scrolled ? "" : " border-gray-700 border"
          }`}
        >
          {navLinks.map((link, idx) => {
            const isActive =
              ...
              <div
                ...
                className={`flex items-center px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 cursor-pointer ${
                  isActive
                    ...
                    : scrolled
                    ? "text-gray-600 hover:text-orange-600 hover:bg-white hover:shadow-sm"
                    : "text-white hover:text-orange-400 hover:bg-white/20"
                }`}
              >
                {link.label}
                {!link.noIcon && (
